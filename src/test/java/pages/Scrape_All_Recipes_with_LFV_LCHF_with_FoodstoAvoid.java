...
public class Scrape_All_Recipes_with_LFV_LCHF_with_FoodstoAvoid {
	...
	public Scrape_All_Recipes_with_LFV_LCHF_with_FoodstoAvoid(WebDriver driver, Connection conn,
	...
	public void scrapeAllRecipes() throws Exception {
		...
		int LCHFCounter = 1, LFVCounter = 1, all_recipesCounter = 1, recipeCounter = 0;
		...
		while (true) {
			...
			for (WebElement link : allLinks) {
				if (link.isDisplayed()) // Filteration based on display urls
				{
					// Filteration based on Recipe name
					boolean foodsToAvoid = chkFoodstoavoid(recipeNames.toString(), "", "");
					boolean validLFVRecipe = chkvalidLFVRecipe(recipeNames.toString(), "", "");
					boolean validLCHFRecipe = chkvalidLCHFRecipe(recipeNames.toString(), "", "");

					if (!foodsToAvoid && (validLFVRecipe || validLCHFRecipe)) {
						recipeNames.add(link.getText());
						recipeUrls.add(link.getAttribute("href"));					
						countresp++;
						 if (countresp == 2) break;// 2 recipes from each page
					}
				}
			}
			...
			for (int j = 0; j < recipeUrls.size(); j++) {
				...
				String recipe_Name = recipeNames.get(j), recipe_URL = recipeUrls.get(j), recipe_ID = "",
						recipe_Category = "", ingredients = "", preparation_Time = "", cooking_Time = "", tags = "",
						no_of_servings = "", cuisine_category = "", recipe_Description = "", preparation_method = "",
						nutrient_values = "", foodCategory = "Vegan";
				
				// Navigate to page
				driver.navigate().to(recipe_URL);
				// tag
				List<WebElement> tagElements = driver.findElements(By.cssSelector("div.col-md-12 ul.tags-list"));
				...
				List<WebElement> ingredientsSectionList = driver.findElements(By.xpath("//div[@id='ingredients']"));
				...
				boolean foodstoAvoidFlag = chkFoodstoavoid(recipe_Name, preparation_method, tags);
				boolean validLFVRecipe = chkvalidLFVRecipe(recipe_Name, ingredients, tags);
				boolean validLCHFRecipe = chkvalidLCHFRecipe(recipe_Name, ingredients, tags);

				if (!foodstoAvoidFlag && (validLFVRecipe || validLCHFRecipe)) {
						...
						for (String allergyItem : allergiesList) {
							...
							if (!trimmedItem.isEmpty()
									&& ingredients.toLowerCase().contains(trimmedItem.toLowerCase())||
											(ingredients.toLowerCase().contains("paneer")||
											ingredients.toLowerCase().contains("cheese")||
											ingredients.toLowerCase().contains("sour cream"))) {
								if (trimmedItem.equalsIgnoreCase("milk")||ingredients.toLowerCase().contains("Paneer")||ingredients.toLowerCase().contains("cheese")||ingredients.toLowerCase().contains("sour cream")) {
	...
	public boolean chkFoodstoavoid(String recipeName, String preparation_method, String tagItems) {
		boolean foodstoAvoidFlag = false;
		for (String foodstoavoid : recipeFoodsToAvoidlist) {
			if (recipeName.contains(foodstoavoid.trim()) || preparation_method.contains(foodstoavoid.trim())
					|| tagItems.contains(foodstoavoid.trim())) {
				foodstoAvoidFlag = true;
				break;
			}
		}
		return foodstoAvoidFlag;
	}

	private boolean chkvalidLCHFRecipe(String recipeName, String ingredientsList, String tagItems) {
		boolean validLCHFRecipe = true;
		for (String eliminatedItem : LCHF_EliminateItemList) {

			if (recipeName.contains(eliminatedItem.trim()) || ingredientsList.contains(eliminatedItem.trim())
					|| tagItems.contains(eliminatedItem.trim())) {
				validLCHFRecipe = false;
				break;
			}
		}
		return validLCHFRecipe;
	}

	private boolean chkvalidLFVRecipe(String recipeName, String ingredientsList, String tagItems) {
