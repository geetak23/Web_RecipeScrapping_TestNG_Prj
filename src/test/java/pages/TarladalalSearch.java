...
public class TarladalalSearch {
	...
	public void scrapeAllRecipes() throws Exception {
		...
		int foodcategoryCounter = 1;// click home button, once first search completes
			...
			if (number > 0) {
				...
				while (true) {
					...
					for (int j = 0; j < recipeUrls.size(); j++) {
						...
						// Iterate LFV Elimination array list using for loop and compare each value
						...
						if (validLFVRecipe) {
							...
							for (String allergyItem : allergiesList) {
								if (ingredients.contains(allergyItem) && allergyItem.trim() != ""
										&& allergyItem.trim().toLowerCase() == "milk") {

									allergyMilk = true;
									System.out.println("Allgent is present in the LFV recipe : " + allergyItem);
									break;
								} else if (ingredients.contains(allergyItem) && allergyItem.trim() != ""
										&& allergyItem.trim().toLowerCase() != "milk") {
									allergyNut = true;
									System.out.println("Allgent is present in the LFV recipe : " + allergyItem);
									break;
							...
							if (allergyMilk) {
								recipes_Allergy_Milk.put(Integer.toString(LFVCounter),
										new Object[] { recipe_ID, recipe_Name, recipe_Category, food_Category,
												ingredients, preparation_Time, cooking_Time, tags, no_of_servings,
												cuisine_category, recipe_Description, preparation_method,
							...
							if (allergyNut) {
								recipes_Allergy_Nut.put(Integer.toString(LFVCounter),
										new Object[] { recipe_ID, recipe_Name, recipe_Category, food_Category,
												ingredients, preparation_Time, cooking_Time, tags, no_of_servings,
												cuisine_category, recipe_Description, preparation_method,
												nutrient_values, recipe_URL, "" });
						...
						if (validLCHFRecipe) {
							...
							/*
							 * boolean allergyMilk = false; boolean allergyNut = false; for (String
							 * allergyItem : allergiesList) { // Then compare each value with Ingredients.
							 * if (ingredients.contains(allergyItem) && allergyItem.trim() != "" &&
							 * allergyItem.trim().toLowerCase()=="milk") {
							 * 
							 * allergyMilk = true;
							 * System.out.println("Allgent Milk is present in the LCHF recipe."); break; }
							 * else if (ingredients.contains(allergyItem) && allergyItem.trim() != ""&&
							 * allergyItem.trim().toLowerCase()!="milk"&&
							 * allergyItem.trim().toLowerCase()!="soy") { allergyNut = true;
							 * System.out.println("Allgent Nut is present in the LCHF recipe:" +
							 * allergyItem); break; } } if(allergyMilk) {
							 * recipes_Allergy_Milk.put(Integer.toString(LCHFCounter), new Object[] {
							 * recipe_ID, recipe_Name, recipe_Category, food_Category, ingredients,
							 * preparation_Time, cooking_Time, tags, no_of_servings, cuisine_category,
							 * recipe_Description, preparation_method, nutrient_values, recipe_URL, "" }); }
							 * if(allergyNut) { recipes_Allergy_Nut.put(Integer.toString(LCHFCounter), new
							 * Object[] { recipe_ID, recipe_Name, recipe_Category, food_Category,
							 * ingredients, preparation_Time, cooking_Time, tags, no_of_servings,
							 * cuisine_category, recipe_Description, preparation_method, nutrient_values,
							 * recipe_URL, "" }); }
							 */
		...
		System.out.println("Total Allergy Milk Recipes = " + recipes_Allergy_Milk.size());
		System.out.println("Total Allergy Nut Recipes = " + recipes_Allergy_Nut.size());
		System.out.println("********************************************");
		...
		// Insert data for Allent Recipes into PostgreSQL
		dbQuries.insertRow(conn, "lfv_recipes_allergy_with_milk", recipes_Allergy_Milk);
		dbQuries.insertRow(conn, "lfv_recipes_allergy_with_nut", recipes_Allergy_Nut);
	}
	...
	public void read_LFV_Data_Excel() {
		...
		System.out.println("Is the Datasheet exist for Allergies? -  " + allergysheetCheck);
		for (int i = 2; i <= 14; i++) {
