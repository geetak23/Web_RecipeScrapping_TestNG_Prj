...
public class Search_Scrape_Recipes_with_LFV_LCHF {
	...
	public Search_Scrape_Recipes_with_LFV_LCHF(WebDriver driver, Connection conn, PostgresqlQueries dbQuries) {
		this.driver = driver;
		this.conn = conn;
	...
	public void scrapeAllRecipes() throws Exception {
		...
		for (int foodCatIndex = 0; foodCatIndex < foodCategoryDataList.size(); foodCatIndex++) {
			...
			System.out.println("\nNumber of " + foodCategory + " recipes displayed in the search are : " + totalRecipes); 
					...
					for (int j = 0; j < recipeUrls.size(); j++) {
						...
						if (validLFVRecipe || validLCHFRecipe) {
							...
							try {
								cooking_Time = driver
										.findElement(
												By.xpath("//h6[text()='Cooking Time']/following-sibling::p/strong"))
										.getText();
							} catch (Exception e) {
							}
							// No of servings
							...
							List<WebElement> nutrientElements = driver
									.findElements(By.xpath("(//div[@id='rcpnuts'])[1]"));
			...
			System.out.println("\nTotal number of " + foodCategory + " recipes scrapped for LFV and LCHF are: " + (recipeCounter));
		}
